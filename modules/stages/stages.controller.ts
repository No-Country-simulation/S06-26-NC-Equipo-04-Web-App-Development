...
export const create = asyncHandler(async (req: Request, res: Response) => {
  const stage = await stagesService.addStage(req.params.id, req.body, req.user!.id);
  res.status(201).json({ success: true, data: stage, message: 'Etapa agregada' });
});

export const list = asyncHandler(async (req: Request, res: Response) => {
  const stages = await stagesService.getStages(req.params.id);
  res.status(200).json({ success: true, data: stages });
});

export const update = asyncHandler(async (req: Request, res: Response) => {
  const stage = await stagesService.updateStage(req.params.id, req.params.stageId, req.body, req.user!.id);
  res.status(200).json({ success: true, data: stage, message: 'Etapa actualizada' });
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
  await stagesService.deleteStage(req.params.id, req.params.stageId, req.user!.id);
